...
bool Application::mainLoop()
{
    ...
    // Analog stick -> dpad.
    //
    // Upstream legacy borealis never read the sticks (there was just a TODO
    // here), so pushing the stick did nothing at all and the app looked
    // frozen. Both sticks are folded into the dpad buttons *before* the
    // button loop below, so the existing press/repeat logic applies to them
    // unchanged. A dpad press always wins over the stick, so nothing changes
    // for dpad users.
    {
        const float STICK_THRESHOLD = 0.5f;

        auto stickToDpad = [](float value, char negativeButton, char positiveButton, float threshold) {
            if (value <= -threshold)
                Application::gamepad.buttons[negativeButton] = GLFW_PRESS;
            else if (value >= threshold)
                Application::gamepad.buttons[positiveButton] = GLFW_PRESS;
        };

        float leftX  = Application::gamepad.axes[GLFW_GAMEPAD_AXIS_LEFT_X];
        float leftY  = Application::gamepad.axes[GLFW_GAMEPAD_AXIS_LEFT_Y];
        float rightX = Application::gamepad.axes[GLFW_GAMEPAD_AXIS_RIGHT_X];
        float rightY = Application::gamepad.axes[GLFW_GAMEPAD_AXIS_RIGHT_Y];

        // GLFW's Y axis points down, so -Y is up.
        stickToDpad(leftX, GLFW_GAMEPAD_BUTTON_DPAD_LEFT, GLFW_GAMEPAD_BUTTON_DPAD_RIGHT, STICK_THRESHOLD);
        stickToDpad(leftY, GLFW_GAMEPAD_BUTTON_DPAD_UP, GLFW_GAMEPAD_BUTTON_DPAD_DOWN, STICK_THRESHOLD);
        stickToDpad(rightX, GLFW_GAMEPAD_BUTTON_DPAD_LEFT, GLFW_GAMEPAD_BUTTON_DPAD_RIGHT, STICK_THRESHOLD);
        stickToDpad(rightY, GLFW_GAMEPAD_BUTTON_DPAD_UP, GLFW_GAMEPAD_BUTTON_DPAD_DOWN, STICK_THRESHOLD);
    }
