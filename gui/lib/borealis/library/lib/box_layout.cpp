...
View* BoxLayout::getNextFocus(FocusDirection direction, View* currentView)
{
    ...
    // A view that isn't one of our children has no index stored in its parent
    // user data. Upstream dereferenced it unconditionally below, which is a
    // null dereference (an instant crash on Switch) whenever focus traversal
    // walks through a view added via setContentView() instead of addView().
    if (!parentUserData)
        return nullptr;
